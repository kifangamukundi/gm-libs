...
type Cache interface {
	...
	SetCache(cacheKey string, data any, ttl time.Duration) error
}
...
func (r *RedisCache) SetCache(cacheKey string, data any, ttl time.Duration) error {
...
func (r *RedisCache) SetBinaryCache(cacheKey string, data any, ttl time.Duration) error {
...
func (r *RedisCache) ScanKeys(pattern string, count int64) ([]string, uint64, error) {
	return r.client.Scan(context.Background(), 0, pattern, count).Result()
}
